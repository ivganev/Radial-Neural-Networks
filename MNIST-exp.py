...
####################################
####### Get sample info & make sample with label 3
####################################
...
noisy_threes, noisy_labels = add_noise(label=3, n=num_samples, m=m_copies, verbose =False)

noisy_threes_flat = noisy_threes.flatten(1)

####################################
####### Get dimension vector information
####################################

print('The input dimension is 28*28 and the output dimension is 1')
print('Enter the hidden widths, separated by commas:')
hidden_dims = list(map(int, input().split(',')))
dim_vector = [28*28] + hidden_dims + [1]
print('The full dimension vector is:', dim_vector)
print('')
...
print('')
print('###Training stepReLU radial network:')
